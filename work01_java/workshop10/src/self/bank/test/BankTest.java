...
public class BankTest {
	...
	private static void addCustomer() {
		...
		String city = sc.next();
		// 2.입력받은 값으로 Customer생성 Customer cust = new Customer(name, ssn, new
		...
		// 3. 배열에 생성된 고객을 추가...서비스 기능.. BankService.getInstance().addCustomer(cust);
	...
	private static void createAccount() {
		...
		int balance = 0; // 1.입력받은 ssn에 해당하는 고객이 있다면 통장개설을 시작하려한다...
	...
	private static void showCustomer() {// 특정회원에 대한 정보를 출력..
		...
		Scanner sc = new Scanner(System.in);
		int ssn = sc.nextInt(); // 1. 주민번호에 해당하는 고객이 존재한다면 정보를 출력함
	...
	public static void showBalance(int ssn) { // 고객의 잔액을 출력한다.
		System.out.println(BankService.getInstance().getCustomer(ssn).getAccount());
	}
